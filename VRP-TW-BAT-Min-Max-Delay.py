...
def arcLength(time,batteryLevel,headTimeWindow,headBatteryWindow,travelTime,batteryDelta1,batteryDelta2,loadFactor,batteryCapacity):
    ...
    cost = max(0,arrivalTime-headTimeWindow[1])
...
class VRPmodelHandler():
    ...
    def fillModel(self,varType = 'B'):
        ...
        self.addVariables(["gamma"],["C"],[1],[0],[self.vrp.timeHorizon*self.vrp.n],'gamma')
        ...
        for node in self.vrp.nodes:
            thevars = [arc.getCurrentVarName() for interval_node in node.timeNodes for bat_node in interval_node.batteryNodes
                       for arc in bat_node.outgoingArcs]+["gamma"]
            thecoefs = [arc.cost for interval_node in node.timeNodes for bat_node in interval_node.batteryNodes
                       for arc in bat_node.outgoingArcs]+[-1.0]
            if len(thevars)>0:
                allvars.append(cplex.SparsePair(thevars,thecoefs))
                allsenses.append("L")
                allrhs.append(0.0)
                allNames.append("cost_%d" % node.name)
    ...
    def changeVariableName(self,oldVarName,newVarName):
        ...
        if newVarName[0] == 'y':
            ...
            self.yNames[idx-len(self.xNames)-1]=newVarName
    ...
    def solveLpRelaxation(self,node,tol=0.00001):
        ...
        if not feas:
            ...
            node.primalXValues = { name:val for name,val in zip(self.xNames,node.primalValues[1:self.xNum+1])
                        if node.primalValues[self.name2idx[name]]>tol }
...
class VRPgraph():
    ...
    def findSplitPoints(self,sol):#heuristic for additional split points 
        splitPointList = []
        for path in sol:
            pathTB = self.findTimedBatteryPath(path)
...
class Arc():
    ...
    def createVarInModel(self,modelHandler):
        ...
        modelHandler.addVariables([self.getCurrentVarName()],['C'],[0],[0.0],[1.0],'y')
    def registerToModel(self,modelHandler):
        ...
        modelHandler.changeConstraintCoefficient('cost_%d'%(self.tail.name),self.getCurrentVarName(), self.cost)
...
class Tree():
    ...
    #@registerTime
    #@registerCalls
    def chooseNode(self,selection=2):
        if selection == 1:
            minInd = 0
            minVal= 100000000
        ...
        if selection == 2:
            ...
            minLb = 100000000
            for i,node in enumerate(self.openNodes):
                if node.lowerBound - node.getOneBranches() < minVal:
                    minInd = i
                    minVal = node.lowerBound - node.getOneBranches() 
    ...
    def splitAtRoot(self):
        ...
        while (abs(prevLb-lb)> 0.1 or count < 20) and len(splitPointList)>0:
            ...
            paths,shortCycles = self.vrp.solToCycles(self.root.primalYValues)
            if len(shortCycles) == 0:
                ...
                if objective < self.ub:
                    ...
                    self.ub = objective+0.1
                    self.heuristicSol = timedBatteryPaths
                    self.heuristicUb = objective
            count += 1
            splitPointList = self.vrp.solToArcCorrections(self.root.primalYValues)
    ...
    def branchAndRefine(self):
        ...
        self.splitAtRoot()
        return
        while ( len(self.openNodes)>0) and (time.time()-t0 < self.timeLimit):
            self.count+=1
            ...
            if node.lowerBound < self.ub-0.99:
    ...
    for tour in tours:
        for node in tour:
            if node[1]>vrp.nodes[node[0]].timeWindow[1]:
                print(f"Time-window violation of {node[1]-vrp.nodes[node[0]].timeWindow[1]}")

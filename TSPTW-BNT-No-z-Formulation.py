        ...
        for node in self.nodes:
            for interval_node in node.interval_nodes:
                if node.name != self.goal and node.name !=self.depot:
                    ...
                    thecoefs = [-1]*len(interval_node.outgoing_arcs)
                    ...
                    thecoefs += [1]*len(interval_node.ingoing_arcs)
    
                        
                    allvars.append(cplex.SparsePair(thevars,thecoefs))
                    allsenses.append("E")
                    allrhs.append(0.0)
                    all_names.append("flow_%d_%d" %(node.name,interval_node.id))
...
class Tsp_node():
    ...
    def split_node(self,split_point,dual_value_locations=[]):
        ...
        if self.tsp.adapt_model:
            if self.name!=self.tsp.goal:
                names = ["flow_%d_%d" %(self.name,self.interval_nodes[idx+1].id)]
                lin_expr=[cplex.SparsePair([],[] )]
                self.tsp.add_constraints(names,lin_expr,["E"],[0.0])
                if self.tsp.update_duals:
                    dual_value_locations += [self.tsp.const_name2idx["flow_%d_%d"%(self.name,self.interval_nodes[idx].id)]]
            
        pop_indices = []
        arc_index = 0
        
        for arc in self.interval_nodes[idx].ingoing_arcs:
            #print "Old arc: %d,%d,%d,%d" %(arc.tail.name,arc.head.name,arc.tail.id,arc.head.id)
            if not arc.head.is_lowest_reachable(arc.tail,arc.length):
                ...
                if self.tsp.adapt_model:
                    ...
                    if self.name!=self.tsp.goal:
                        const_name = "flow_%d_%d" % (self.name,self.interval_nodes[idx].id)
                        self.tsp.model.linear_constraints.set_coefficients([(const_name,old_var_name,0.0)]) 
                        
                        const_name = "flow_%d_%d" % (self.name,self.interval_nodes[idx+1].id)
            ...
            if not arc.head.is_lowest_reachable(arc.tail,arc.length):
                ...
                if arc.head.is_lowest_reachable(self.interval_nodes[idx+1],
                    ...
                    if self.tsp.adapt_model and (not self.tsp.check_interval_precedences or all_nodes_reachable or 1):
                        ...
                        if new_arc.head.name != self.tsp.goal:
                            const_name = "flow_%d_%d" % (new_arc.head.name,new_arc.head.id)
                            self.tsp.model.linear_constraints.set_coefficients([(const_name,new_var_name,1.0)])
                        if new_arc.tail.name != self.tsp.depot:
                            const_name = "flow_%d_%d" % (new_arc.tail.name,new_arc.tail.interval[0])
                            self.tsp.model.linear_constraints.set_coefficients([(const_name,new_var_name,-1.0)]) 
                    ...
                    if is_reachable:
                        ...
                        if self.tsp.adapt_model and (not self.tsp.check_interval_precedences or all_nodes_reachable or 1):
                            new_var_name = "y_%d_%d_%d_%d" % (new_arc.tail.name,new_arc.head.name,
                                                              new_arc.tail.id,new_arc.head.id)
                            #print "new_arc: %d,%d,%d,%d" %(new_arc.tail.name, new_arc.head.name,
                            #                               new_arc.tail.id,new_arc.head.id)
                            if (not self.tsp.check_interval_precedences or all_nodes_reachable):
                                self.tsp.add_variables([new_var_name],['C'],[0.0],[0.0],[1.0],'y')
                            else:
                                self.tsp.add_variables([new_var_name],['C'],[0.0],[0.0],[0.0],'y')
                            if new_arc.head.name != self.tsp.goal:   
                                const_name = "flow_%d_%d" % (new_arc.head.name,new_arc.head.interval[0])
                                self.tsp.model.linear_constraints.set_coefficients([(const_name,new_var_name,1.0)]) 
                            if new_arc.tail.name != self.tsp.depot:
                                const_name = "flow_%d_%d" % (new_arc.tail.name,new_arc.tail.interval[0])
                                self.tsp.model.linear_constraints.set_coefficients([(const_name,new_var_name,-1.0)])
...
#"rbg027a.tw":5091,	
#"rbg016a.tw":938,#	"rbg021.2.tw":4528,	
#"rbg031a.tw":1863,	
#"rbg016b.tw":1304,#	"rbg021.3.tw":4528,	"rbg033a.tw":2069,	

...
def writeData(file_name,TWs,distM,costM,direcotry_name ="SPPTW_iterative_instances"):
...
def generateInstance(levels=300,pointsPerLevel=5,width=5.0,length = 100.0,arcsPerPoint=5,rando=0):
    ...
    for k in range(1,levels+1):
        ...
        if shifter > 1:
            ...
            predecessors +=[[] for i in range(pointsPerLevel)]
            for j in range(shifter-pointsPerLevel,shifter):
                for i in range(arcsPerPoint):
                        ...
                        predecessors[chosenOne].append(j)
        else:
    ...
    for j in range(shifter-pointsPerLevel,shifter):
        ...
        predecessors[len(points)-1].append(j)
    successors.append([])
    ...
    for i in range(len(points)):
        for j in successors[i]:
            ...
            if not rando:
                ...
                costMatrix[i][j] = random.randint(1,5)
            if costMatrix[i][j] == 0:
                costMatrix[i][j] = 1
    TWs=[]
    for i,p in enumerate(points):
        if i == 0:    
            ...
            TWs.append([int(levels*points[i][1]/length+0.0001),int(levels*points[-1][1]/length+1)*int(width)])
    return distMatrix,points,successors,predecessors,costMatrix,distList,TWs

sys.setrecursionlimit(1500)
for MM in range(51,71):
    step = 5
    distM,points,successors,predecessors,costM,distList,TWs = generateInstance(levels=MM,
                                        pointsPerLevel=5,width=5.0,length = 50,arcsPerPoint=5,rando=1)  
    processed = [0]
    ...
    processed = [len(points)-1]
    toBeUpdated = [i for i in predecessors[len(points)-1]]
    while toBeUpdated != []:
        for i in toBeUpdated:
            latAtime=0
            for j in processed:
                if i in predecessors[j]:
                    if TWs[j][1]-distM[i][j] > latAtime:
                        latAtime=TWs[j][1]-distM[i][j]
            if latAtime < TWs[i][1]:
                TWs[i][1] = latAtime
        processed += toBeUpdated
        NewtoBeUpdated = []
    ...
    iteras =1
    while iteras==1:
        vert_num,adj_matrix,cost_matrix = len(points),distM,costM
        vertNum=vert_num
        nodes = [[i,TWs[i][0],TWs[i][1]] for i in range(vertNum)]
        ...
        print iteras
        if iteras == 1:
            writeData("n_%d_a_5_iter_0" % (len(TWs)),TWs,distM,costM,"SPPTW_iterative_instances")
            for j,i in enumerate(TWs):
                if i[1]-step>i[0]:
                    i[1] -= step
    ...
    I=1
    while 1 and I < 80:
        writeData("n_%d_a_5_iter_%d" % (len(TWs),I),TWs,distM,costM,"SPPTW_iterative_instances")
        if TWs[-1][1]-step<=TWs[-1][0]:
            step=1
            TWs[-1][1] = TWs[-1][0]
            vert_num,adj_matrix,cost_matrix = len(points),distM,costM
            vertNum=vert_num
